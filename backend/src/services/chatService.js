...
import { queryContent } from "./pineconeService.js";
...
const retrieveForType = async (type, message, user, topK = RESULTS_PER_TYPE) => {
  ...
  return hits.map((h) => h.fields?.text || h.text).filter(Boolean);
};
...
export const handleChatMessage = async (message, user = null) => {
  ...
  if (!contextBlocks.length && !isGuest) {
    ...
    const grouped = {};
    hits.forEach((h) => {
      const type = h.fields?.type || h.type;
      const text = h.fields?.text || h.text;
      if (!type || !text) return;
